...
int aws_date_time_init_from_str(struct aws_date_time *dt,
    ...
    dt->timestamp = (uint64_t)mktime(&parsed_time);
}

int aws_date_time_to_local_time_str(struct aws_date_time *dt, enum aws_date_format fmt,
                                    struct aws_byte_buf *output_buf) {
    struct tm time;
    localtime_r(&dt->timestamp, &time);
...
uint8_t aws_date_time_sec(struct aws_date_time *dt, bool local_time);

bool aws_date_time_dst(struct aws_date_time *dt, bool local_time);

uint64_t aws_date_time_diff(struct aws_date_time *a, struct aws_date_time *b);