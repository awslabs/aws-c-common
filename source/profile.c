    ...
    *current_profile_out = NULL;
    ...
    if (profile_collection->profile_source == AWS_PST_CONFIG && s_is_default_profile_name(profile_name)) {
        ...
        if (has_prefix && existing_profile && !existing_profile->has_profile_prefix) {
             ...
             */
            AWS_LOGF_WARN(
                AWS_LS_COMMON_PROFILE, "Prefixed default config profile replacing unprefixed default profile");
            s_log_parse_context(AWS_LL_WARN, context);
...
struct aws_profile_collection *aws_profile_collection_new_from_merge(
    ...
    if (config_profiles != NULL) {
        if (s_profile_collection_merge(merged, config_profiles)) {
            AWS_LOGF_ERROR(AWS_LS_COMMON_PROFILE, "Failed to merge config profile set");
            goto cleanup;
        }
    }
    ...
    if (credentials_profiles != NULL) {
        if (s_profile_collection_merge(merged, credentials_profiles)) {
            AWS_LOGF_ERROR(AWS_LS_COMMON_PROFILE, "Failed to merge credentials profile set");
            goto cleanup;
        }
    }
...
static bool s_parse_profile_declaration(
    ...
    if (has_profile_prefix) {
        if (context->profile_collection->profile_source == AWS_PST_CREDENTIALS) {
            ...
            context->parse_error = AWS_ERROR_PARSE_RECOVERABLE_ERROR;
            return true;
        }
    ...
    if (!s_parse_by_character_predicate(&profile_cursor, s_is_identifier, &profile_name, 0)) {
        AWS_LOGF_WARN(AWS_LS_COMMON_PROFILE, "Profile declarations must contain a valid identifier for a name");
        s_log_parse_context(AWS_LL_WARN, context);
    ...
    if (context->profile_collection->profile_source == AWS_PST_CONFIG && !has_profile_prefix &&
        ...
        AWS_LOGF_WARN(
            AWS_LS_COMMON_PROFILE, "Non-default profile declarations in config files must use the \"profile\" keyword");
        s_log_parse_context(AWS_LL_WARN, context);
    ...
    if (profile_cursor.len == 0) {
        AWS_LOGF_WARN(AWS_LS_COMMON_PROFILE, "Profile declaration missing required ending bracket");
        s_log_parse_context(AWS_LL_WARN, context);

        context->parse_error = AWS_ERROR_PARSE_FATAL_ERROR;
        return true;
    }
    ...
    if (s_profile_collection_add_profile(
        ...
        AWS_LOGF_ERROR(AWS_LS_COMMON_PROFILE, "Failed to add profile to profile collection");
...
static bool s_parse_property_continuation(
    ...
    if (continuation_cursor.len == 0) {
        AWS_LOGF_ERROR(AWS_LS_COMMON_PROFILE, "Property continuation internal parsing error");
        s_log_parse_context(AWS_LL_ERROR, context);
    ...
    if (context->current_profile == NULL || context->current_property == NULL) {
        AWS_LOGF_WARN(AWS_LS_COMMON_PROFILE, "Property continuation seen outside of a current property");
        s_log_parse_context(AWS_LL_WARN, context);
    ...
    if (s_profile_property_add_continuation(context->current_property, &continuation_cursor)) {
        AWS_LOGF_WARN(AWS_LS_COMMON_PROFILE, "Property continuation could not be applied to the current property");
        s_log_parse_context(AWS_LL_WARN, context);
    ...
    if (context->current_property->is_empty_valued) {
        ...
        if (!s_parse_by_character_predicate(&continuation_cursor, s_is_not_assignment_operator, &key_cursor, 0)) {
            AWS_LOGF_WARN(
                AWS_LS_COMMON_PROFILE, "Empty-valued property continuation must contain the assignment operator");
            s_log_parse_context(AWS_LL_WARN, context);
        ...
        if (s_profile_property_add_sub_property(
            ...
            AWS_LOGF_ERROR(AWS_LS_COMMON_PROFILE, "Internal error adding sub property to current property");
            s_log_parse_context(AWS_LL_ERROR, context);
...
static bool s_parse_property(const struct aws_byte_cursor *line_cursor, struct profile_file_parse_context *context) {
    ...
    if (!s_parse_by_character_predicate(&property_cursor, s_is_not_assignment_operator, &key_cursor, 0)) {
        AWS_LOGF_WARN(AWS_LS_COMMON_PROFILE, "Property definition does not contain the assignment operator");
        s_log_parse_context(AWS_LL_WARN, context);
    ...
    struct aws_byte_cursor id_check_cursor = aws_byte_cursor_trim_pred(&trimmed_key_cursor, s_is_identifier);
    if (id_check_cursor.len > 0) {
        AWS_LOGF_WARN(AWS_LS_COMMON_PROFILE, "Property definition does not begin with a valid identifier");
        s_log_parse_context(AWS_LL_WARN, context);
    ...
    if (context->current_profile != NULL) {
        ...
        if (context->has_seen_profile) {
            AWS_LOGF_WARN(AWS_LS_COMMON_PROFILE, "Property definition seen outside a profile");
            s_log_parse_context(AWS_LL_WARN, context);

            context->parse_error = AWS_ERROR_PARSE_RECOVERABLE_ERROR;
        } else {
            AWS_LOGF_WARN(AWS_LS_COMMON_PROFILE, "Property definition seen before any profiles");
            s_log_parse_context(AWS_LL_WARN, context);
...
static void s_parse_and_apply_line_to_profile_collection(
    ...
    AWS_LOGF_WARN(AWS_LS_COMMON_PROFILE, "Unidentifiable line type encountered while parsing profile file");
    s_log_parse_context(AWS_LL_WARN, context);
...
static struct aws_profile_collection *s_aws_profile_collection_new_internal(
    ...
    if (current_position.len > 0) {
        ...
        while (aws_byte_cursor_next_split(&current_position, '\n', &line_cursor)) {
            ...
            if (context.parse_error == AWS_ERROR_PARSE_FATAL_ERROR) {
                AWS_LOGF_WARN(AWS_LS_COMMON_PROFILE, "Fatal error while parsing aws profile collection");
                goto cleanup;
            }
...
struct aws_profile_collection *aws_profile_collection_new_from_file(
    ...
    AWS_LOGF_DEBUG(AWS_LS_COMMON_PROFILE, "Creating profile collection from file at \"%s\"", file_path->bytes);

    if (aws_byte_buf_init_from_file(&file_contents, allocator, aws_string_c_str(file_path)) != 0) {
        AWS_LOGF_WARN(AWS_LS_COMMON_PROFILE, "Failed to read file at \"%s\"", file_path->bytes);
        return NULL;
    }
