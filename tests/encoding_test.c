...
static int run_hex_encoding_test_case(struct aws_allocator *alloc, const char *test_str, size_t test_str_size,
    ...
    ASSERT_SUCCESS(aws_hex_encode((uint8_t *)test_str, test_str_size - 1, output_str, output_size),
                   "encode call should have succeeded");

    ASSERT_BIN_ARRAYS_EQUALS(expected, expected_size, output_str, output_size, "Encode output should have been %s.",
                             expected);
    ASSERT_INT_EQUALS(*allocation, (char)0xdd, "Write should not have occurred before the start of the buffer.");
    ASSERT_INT_EQUALS(*(allocation + output_size + 1), (char)0xdd,
    ...
    ASSERT_SUCCESS(aws_hex_decode(expected, expected_size - 1, (uint8_t *)output_str, output_size),
                   "decode call should have succeeded");

    ASSERT_BIN_ARRAYS_EQUALS(test_str, test_str_size - 1, output_str, output_size, "Decode output should have been %s.",
                             test_str);
    ASSERT_INT_EQUALS(*allocation, (char)0xdd, "Write should not have occurred before the start of the buffer.");
    ASSERT_INT_EQUALS(*(allocation + output_size + 1), (char)0xdd,
                      "Write should not have occurred after the start of the buffer.");
...
static int hex_encoding_test_case_missing_leading_zero_fn(struct aws_allocator *alloc, void *ctx) {
    ...
    ASSERT_SUCCESS(aws_hex_decode(test_data, sizeof(test_data) - 1, output, sizeof(output)), "Hex decoding failed with "
                                                                                             "error code %d",
                   aws_last_error());

    ASSERT_BIN_ARRAYS_EQUALS(expected, sizeof(expected), output, sizeof(output),
...
static int hex_encoding_invalid_buffer_size_test_fn(struct aws_allocator *alloc, void *ctx) {
    ...
    ASSERT_ERROR(AWS_ERROR_INVALID_BUFFER_SIZE,
                 aws_hex_encode((const uint8_t *)test_data, sizeof(test_data), output_buff, size_too_small),
                 "Invalid buffer size should have failed with AWS_ERROR_INVALID_BUFFER_SIZE");

    ASSERT_ERROR(AWS_ERROR_INVALID_BUFFER_SIZE,
                 aws_hex_decode(test_data, sizeof(test_data), (uint8_t *)output_buff, size_too_small),
                 "Invalid buffer size should have failed with AWS_ERROR_INVALID_BUFFER_SIZE");
    return 0;
}
...
static int hex_encoding_overflow_test_fn(struct aws_allocator *alloc, void *ctx) {
    ...
    ASSERT_ERROR(AWS_ERROR_OVERFLOW_DETECTED,
                 aws_hex_encode((const uint8_t *)test_data, overflow, output_buff, sizeof(output_buff)),
...
static int hex_encoding_invalid_string_test_fn(struct aws_allocator *alloc, void *ctx) {
    ...
    ASSERT_ERROR(AWS_ERROR_INVALID_HEX_STR,
                 aws_hex_decode((const char *)bad_input, sizeof(bad_input) - 1, output, sizeof(output)),
...
static int run_base64_encoding_test_case(struct aws_allocator *alloc, const char *test_str, size_t test_str_size,
    ...
    ASSERT_SUCCESS(aws_base64_encode((uint8_t *)test_str, test_str_size, output_str, output_size),
                   "encode call should have succeeded");
    ...
    ASSERT_SUCCESS(aws_base64_decode(expected, expected_size - 1, (uint8_t *)output_str, output_size),
                   "decode call should have succeeded");

    ASSERT_BIN_ARRAYS_EQUALS(test_str, test_str_size, output_str, output_size, "Decode output should have been %s.",
                             test_str);
    ASSERT_INT_EQUALS(*allocation, (char)0xdd, "Write should not have occurred before the start of the buffer.");
    ASSERT_INT_EQUALS(*(allocation + output_size + 1), (char)0xdd,
                      "Write should not have occurred after the start of the buffer.");
...
static int base64_encoding_buffer_size_too_small_test_fn(struct aws_allocator *alloc, void *ctx) {
    ...
    ASSERT_ERROR(AWS_ERROR_INVALID_BUFFER_SIZE,
                 aws_base64_encode((const uint8_t *)test_data, sizeof(test_data) - 1, output_buff, size_too_small),
                 "Invalid buffer size should have failed with AWS_ERROR_INVALID_BUFFER_SIZE");

    ASSERT_ERROR(AWS_ERROR_INVALID_BUFFER_SIZE,
                 aws_base64_decode(encoded_data, sizeof(encoded_data) - 1, (uint8_t *)output_buff, size_too_small),
                 "Invalid buffer size should have failed with AWS_ERROR_INVALID_BUFFER_SIZE");
    return 0;
}
...
static int base64_encoding_buffer_size_overflow_test_fn(struct aws_allocator *alloc, void *ctx) {
    ...
    ASSERT_ERROR(AWS_ERROR_OVERFLOW_DETECTED,
                 aws_base64_encode((const uint8_t *)test_data, overflow + 2, output_buff, sizeof(output_buff)),
                 "overflow buffer size should have failed with AWS_ERROR_OVERFLOW_DETECTED");

    ASSERT_ERROR(AWS_ERROR_OVERFLOW_DETECTED,
                 aws_base64_decode(encoded_data, overflow, (uint8_t *)output_buff, sizeof(output_buff)),
                 "overflow buffer size should have failed with AWS_ERROR_OVERFLOW_DETECTED");
    return 0;
}
...
static int base64_encoding_buffer_size_invalid_test_fn(struct aws_allocator *alloc, void *ctx) {
    ...
    ASSERT_ERROR(AWS_ERROR_INVALID_BASE64_STR,
                 aws_base64_decode(encoded_data, sizeof(encoded_data), (uint8_t *)output_buff, sizeof(output_buff)),
...
static int base64_encoding_invalid_buffer_test_fn(struct aws_allocator *alloc, void *ctx) {
    ...
    char encoded_data[] = "Z\n9vYmFy";
    char output_buff[sizeof(encoded_data)] = {0};

    ASSERT_ERROR(AWS_ERROR_INVALID_BASE64_STR,
                 aws_base64_decode(encoded_data, sizeof(encoded_data) - 1, (uint8_t *)output_buff, sizeof(output_buff)),
...
static int uint24_buffer_non_aligned_test_fn(struct aws_allocator *alloc, void *ctx) {
    ...
    uint8_t expected[] = {0x10, 0x20, 0x30};
    ASSERT_BIN_ARRAYS_EQUALS(expected, sizeof(expected), (buffer + 6), sizeof(buffer) - 6,
...
static int uint16_buffer_non_aligned_test_fn(struct aws_allocator *alloc, void *ctx) {
    ...
    uint8_t expected[] = {0x10, 0x20};
    ASSERT_BIN_ARRAYS_EQUALS(expected, sizeof(expected), (buffer + 7), sizeof(buffer) - 7,
