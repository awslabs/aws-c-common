...
static int array_list_order_push_back_pop_front_fn(
    ...
    ASSERT_SUCCESS(
        aws_array_list_push_back(&list, (void *)&first),
    ...
    ASSERT_SUCCESS(
        ...
        aws_last_error());
    ASSERT_SUCCESS(
        aws_array_list_push_back(&list, (void *)&third),
        "List push failed with error code %d",
        aws_last_error());
    ASSERT_SUCCESS(
        aws_array_list_push_back(&list, (void *)&fourth),
        "List push failed with error code %d",
        aws_last_error());

    ASSERT_INT_EQUALS(
        list_size, list.length, "List size should be %d.", (int)list_size);
    ASSERT_INT_EQUALS(
        list_size,
        list.current_size / sizeof(int),
        "Allocated list size should be %d.",
    ...
    int item;
    ASSERT_SUCCESS(
        aws_array_list_front(&list, (void *)&item),
        "List front failed with error code %d",
        aws_last_error());
    ASSERT_SUCCESS(
        aws_array_list_pop_front(&list),
        "List pop front failed with error code %d",
        aws_last_error());
    ASSERT_INT_EQUALS(first, item, "Item should have been the first item.");
    ASSERT_INT_EQUALS(
        list_size - 1,
        list.length,
        "List size should be %d.",
        (int)list_size - 1);
    ASSERT_INT_EQUALS(
        list_size,
        list.current_size / sizeof(int),
...
AWS_TEST_CASE(
    ...
    array_list_order_push_back_pop_front_fn)
...
static int array_list_order_push_back_pop_back_fn(
    ...
    int item;
    ASSERT_SUCCESS(
        aws_array_list_back(&list, (void *)&item),
        "List back failed with error code %d",
        aws_last_error());
    ASSERT_SUCCESS(
        aws_array_list_pop_back(&list),
        "List pop back failed with error code %d",
        aws_last_error());
    ASSERT_INT_EQUALS(fourth, item, "Item should have been the fourth item.");
    ASSERT_INT_EQUALS(
        list_size - 1,
...
AWS_TEST_CASE(
    ...
    array_list_order_push_back_pop_back_fn)

static int array_list_pop_front_n_fn(
    struct aws_allocator *allocator,
    void *ctx) {
    (void)ctx;
    ...
    ASSERT_SUCCESS(
        aws_array_list_init_dynamic(&list, allocator, 8, sizeof(int)));
    ...
    ASSERT_INT_EQUALS(
        ...
        "List should be empty after popping last 2 items");
...
static int array_list_exponential_mem_model_test_fn(
    struct aws_allocator *allocator,
    void *ctx) {
    (void)ctx;
...
AWS_TEST_CASE(
    ...
    array_list_exponential_mem_model_test_fn)

static int array_list_exponential_mem_model_iteration_test_fn(
    struct aws_allocator *allocator,
    void *ctx) {
    (void)ctx;
    ...
    ASSERT_SUCCESS(
        aws_array_list_set_at(&list, (void *)&first, 0),
        "array list push back failed with error %d",
        aws_last_error());
    ASSERT_INT_EQUALS(list_size, list.current_size / sizeof(int));
    ...
    ASSERT_SUCCESS(
        aws_array_list_set_at(&list, (void *)&third, 2),
        "array list push back failed with error %d",
        aws_last_error());
    ASSERT_INT_EQUALS(
        list_size << 2,
...
AWS_TEST_CASE(
    ...
    array_list_exponential_mem_model_iteration_test_fn)

static int array_list_set_at_overwrite_safety_fn(
    struct aws_allocator *allocator,
    void *ctx) {
    (void)allocator;
    (void)ctx;
    ...
    ASSERT_ERROR(
        ...
        aws_array_list_set_at(&list, (void *)&value, 4));
...
AWS_TEST_CASE(
    ...
    array_list_set_at_overwrite_safety_fn)

static int array_list_iteration_test_fn(
    struct aws_allocator *allocator,
    void *ctx) {
    (void)ctx;
    ...
    ASSERT_SUCCESS(
        aws_array_list_set_at(&list, (void *)&first, 0),
        "Array set failed with error code %d",
        aws_last_error());
    ...
    ASSERT_INT_EQUALS(3, list.length, "List size should be %d.", 3);
    ASSERT_SUCCESS(
        aws_array_list_set_at(&list, (void *)&fourth, 3),
        "Array set failed with error code %d",
        aws_last_error());
    ...
    int item;
    ASSERT_SUCCESS(
        aws_array_list_get_at(&list, (void *)&item, 0),
        "Array set failed with error code %d",
        aws_last_error());
    ASSERT_INT_EQUALS(first, item, "Item should have been the first item.");
    ASSERT_SUCCESS(
        aws_array_list_get_at(&list, (void *)&item, 1),
        "Array set failed with error code %d",
        aws_last_error());
    ASSERT_INT_EQUALS(second, item, "Item should have been the second item.");
    ASSERT_SUCCESS(
        aws_array_list_get_at(&list, (void *)&item, 2),
        "Array set failed with error code %d",
        aws_last_error());
    ASSERT_INT_EQUALS(third, item, "Item should have been the third item.");
    ASSERT_SUCCESS(
        aws_array_list_get_at(&list, (void *)&item, 3),
        "Array set failed with error code %d",
        aws_last_error());
    ASSERT_INT_EQUALS(fourth, item, "Item should have been the fourth item.");
...
static int array_list_iteration_by_ptr_test_fn(
    ...
    int *item;
    ASSERT_SUCCESS(
        aws_array_list_get_at_ptr(&list, (void **)&item, 0),
        "Array set failed with error code %d",
        aws_last_error());
    ASSERT_INT_EQUALS(first, *item, "Item should have been the first item.");
    ASSERT_SUCCESS(
        aws_array_list_get_at_ptr(&list, (void **)&item, 1),
        "Array set failed with error code %d",
        aws_last_error());
    ASSERT_INT_EQUALS(second, *item, "Item should have been the second item.");
    ASSERT_SUCCESS(
        aws_array_list_get_at_ptr(&list, (void **)&item, 2),
        "Array set failed with error code %d",
        aws_last_error());
    ASSERT_INT_EQUALS(third, *item, "Item should have been the third item.");
    ASSERT_SUCCESS(
        aws_array_list_get_at_ptr(&list, (void **)&item, 3),
        "Array set failed with error code %d",
        aws_last_error());
    ASSERT_INT_EQUALS(fourth, *item, "Item should have been the fourth item.");
...
AWS_TEST_CASE(
    ...
    array_list_iteration_by_ptr_test_fn)

static int array_list_preallocated_iteration_test_fn(
    struct aws_allocator *allocator,
    void *ctx) {
    (void)allocator;
    (void)ctx;
    ...
    aws_array_list_init_static(
        &list, (void *)list_data, list_size, sizeof(int));
    ...
    ASSERT_INT_EQUALS(4, list.length, "List size should be %d.", 4);
    ASSERT_FAILS(
        aws_array_list_set_at(&list, (void *)&fourth, 4),
        "Adding element past the end should have failed");
    ASSERT_INT_EQUALS(
        AWS_ERROR_INVALID_INDEX,
        aws_last_error(),
    ...
    ASSERT_INT_EQUALS(fourth, item, "Item should have been the fourth item.");
    ASSERT_FAILS(
        aws_array_list_get_at(&list, (void *)&item, 4),
        "Getting an element past the end should have failed");
    ASSERT_INT_EQUALS(
        AWS_ERROR_INVALID_INDEX,
        aws_last_error(),
        "Error code should have been INVALID_INDEX but was %d",
...
AWS_TEST_CASE(
    ...
    array_list_preallocated_iteration_test_fn)

static int array_list_preallocated_push_test_fn(
    struct aws_allocator *allocator,
    void *ctx) {
    (void)allocator;
    (void)ctx;
    ...
    ASSERT_SUCCESS(
        aws_array_list_push_back(&list, &first),
        "List push failed with error code %d",
        aws_last_error());
    ASSERT_SUCCESS(
        aws_array_list_push_back(&list, &second),
        "List push failed with error code %d",
        aws_last_error());
    ASSERT_SUCCESS(
        aws_array_list_push_back(&list, &third),
        "List push failed with error code %d",
        aws_last_error());
    ASSERT_SUCCESS(
...
AWS_TEST_CASE(
    ...
    array_list_preallocated_push_test_fn)

static int array_list_shrink_to_fit_test_fn(
    struct aws_allocator *allocator,
    void *ctx) {
    (void)ctx;
    ...
    ASSERT_INT_EQUALS(
        list_size,
        list.current_size / sizeof(int),
        "size before shrink should be %d.",
        list_size * sizeof(int));

    ASSERT_SUCCESS(
        aws_array_list_shrink_to_fit(&list),
        "List shrink to fit failed with error code %d",
        aws_last_error());
    ASSERT_INT_EQUALS(2, list.length, "List size should be %d.", 2);
    ASSERT_INT_EQUALS(
        2,
        list.current_size / sizeof(int),
        "Shrunken size should be %d.",
        2 * sizeof(int));

    int item;
    ASSERT_SUCCESS(
        aws_array_list_get_at(&list, &item, 0),
        "Array set failed with error code %d",
        aws_last_error());
    ASSERT_INT_EQUALS(first, item, "Item should have been the first item.");
    ASSERT_SUCCESS(
        aws_array_list_get_at(&list, &item, 1),
        "Array set failed with error code %d",
        aws_last_error());
    ASSERT_INT_EQUALS(second, item, "Item should have been the second item.");
    ASSERT_FAILS(
        aws_array_list_get_at(&list, &item, 2),
        "Getting an element past the end should have failed");
    ASSERT_INT_EQUALS(
        AWS_ERROR_INVALID_INDEX,
        aws_last_error(),
...
static int array_list_shrink_to_fit_static_test_fn(
    struct aws_allocator *allocator,
    void *ctx) {
    (void)allocator;
    (void)ctx;
    ...
    ASSERT_FAILS(
        aws_array_list_shrink_to_fit(&list),
        "List shrink of static list should have failed.");
    ASSERT_INT_EQUALS(
    ...
    ASSERT_PTR_EQUALS(
        &list_data,
        list.data,
        "The underlying allocation should not have changed");
    ASSERT_INT_EQUALS(
        sizeof(list_data),
        list.current_size,
        "List size should not have been changed");
...
AWS_TEST_CASE(
    ...
    array_list_shrink_to_fit_static_test_fn)

static int array_list_clear_test_fn(
    struct aws_allocator *allocator,
    void *ctx) {
    (void)ctx;
    ...
    ASSERT_INT_EQUALS(
        list_size,
        list.current_size / sizeof(int),
        "cleared size should be %d.",
        (int)list_size * sizeof(int));

    int item;
    ASSERT_FAILS(
        aws_array_list_front(&list, (void *)&item),
        "front() after a clear on list should have been an error");
    ASSERT_INT_EQUALS(
        AWS_ERROR_LIST_EMPTY,
        aws_last_error(),
        "Error code should have been LIST_EMPTY but was %d",
        aws_last_error());
...
static int array_list_copy_test_fn(struct aws_allocator *allocator, void *ctx) {
    ...
    ASSERT_SUCCESS(
        aws_array_list_push_back(&list_a, (void *)&first),
        "List push failed with error code %d",
        aws_last_error());
    ASSERT_INT_EQUALS(1, list_a.length, "List size should be %d.", 1);
    ASSERT_SUCCESS(
        aws_array_list_push_back(&list_a, (void *)&second),
        "List push failed with error code %d",
        aws_last_error());
    ...
    ASSERT_SUCCESS(
        ...
        "List copy failed with error code %d",
        aws_last_error());

    int item;
    ASSERT_SUCCESS(
        aws_array_list_get_at(&list_b, (void *)&item, 0),
        "Array set failed with error code %d",
        aws_last_error());
    ASSERT_INT_EQUALS(first, item, "Item should have been the first item.");
    ASSERT_SUCCESS(
        aws_array_list_get_at(&list_b, (void *)&item, 1),
        "Array set failed with error code %d",
        aws_last_error());
    ASSERT_INT_EQUALS(second, item, "Item should have been the second item.");

    ASSERT_INT_EQUALS(
        aws_array_list_length(&list_a),
        aws_array_list_length(&list_b),
        "list lengths should have matched.");
...
static int array_list_swap_contents_test_fn(
    struct aws_allocator *allocator,
    void *ctx) {
    (void)ctx;
    ...
    ASSERT_SUCCESS(aws_array_list_init_dynamic(
        &list_a, allocator, a_capacity, sizeof(int)));
    ...
    int b_capacity = 3;
    ASSERT_SUCCESS(aws_array_list_init_dynamic(
        &list_b, allocator, b_capacity, sizeof(int)));
    ...
    ASSERT_PTR_EQUALS(
        b_buffer,
        a_buffer_after_swap,
        "Lists A and B should have swapped buffer ownership, but did not");
    ...
    ASSERT_INT_EQUALS(
        2,
        aws_array_list_length(&list_a),
        "List A should have taken B's old length");
    ASSERT_INT_EQUALS(
        b_capacity,
        aws_array_list_capacity(&list_a),
        "List A should have taken B's old capacity");
    ASSERT_SUCCESS(
        aws_array_list_get_at(&list_a, &item, 0),
        "List A should have B's old first item");
    ASSERT_INT_EQUALS(b_1, item, "List A should have B's old first item");
    ASSERT_SUCCESS(
        aws_array_list_get_at(&list_a, &item, 1),
        "List A should have B's old second item");
    ASSERT_INT_EQUALS(b_2, item, "List A should have B's old second item");

    ASSERT_INT_EQUALS(
        1,
        aws_array_list_length(&list_b),
        "List B should have taken A's old length");
    ASSERT_INT_EQUALS(
        a_capacity,
        aws_array_list_capacity(&list_b),
        "List B should have taken A's old capacity");
    ASSERT_SUCCESS(
        aws_array_list_get_at(&list_b, &item, 0),
        "List B should have A's old first item");
...
static int array_list_not_enough_space_test_fn(
    struct aws_allocator *allocator,
    void *ctx) {
    (void)ctx;
    ...
    ASSERT_SUCCESS(
        aws_array_list_copy(&list_a, &list_b),
        "Copy from list_a to list_b should have succeeded");
    ASSERT_INT_EQUALS(
        list_a.length,
        list_b.length,
        "List b should have grown to the length of list a");
    ASSERT_INT_EQUALS(
        2 * sizeof(int),
        list_b.current_size,
...
AWS_TEST_CASE(
    ...
    array_list_not_enough_space_test_fn)

static int array_list_not_enough_space_test_failure_fn(
    struct aws_allocator *allocator,
    void *ctx) {
    (void)ctx;
...
AWS_TEST_CASE(
    ...
    array_list_not_enough_space_test_failure_fn)

static int array_list_of_strings_sort_fn(
    struct aws_allocator *allocator,
    void *ctx) {
    (void)ctx;
    ...
    const struct aws_string *strings[] = {empty,
                                          foo,
                                          bar,
                                          foobar,
                                          foo2,
                                          ...
                                          bar_null_food,
                                          bar_null_back};
    const struct aws_string *sorted[] = {empty,
                                         bar,
                                         bar_null_back,
                                         bar_null_food,
                                         bar_food,
    ...
    ASSERT_SUCCESS(
        aws_array_list_init_dynamic(
            &list, allocator, num_strings, sizeof(const struct aws_string *)),
        "List initialization failed with error %d",
        aws_last_error());
    ...
    for (int idx = 0; idx < num_strings; ++idx) {
        ...
        ASSERT_SUCCESS(
            aws_array_list_get_at(&list, (void **)&str, idx),
            "List get failed with error code %d",
            aws_last_error());
        ASSERT_INT_EQUALS(
            0, aws_string_compare(str, sorted[idx]), "Strings should be equal");
    }
    aws_array_list_clean_up(&list);
    return 0;
}
...
static int array_list_empty_sort_fn(
    struct aws_allocator *allocator,
    void *ctx) {
    (void)ctx;
    ...
    ASSERT_SUCCESS(
        aws_array_list_init_dynamic(
            &list, allocator, 5, sizeof(const struct aws_string *)),
        "List initialization failed with error %d",
        aws_last_error());
