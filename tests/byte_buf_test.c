...
static int test_buffer_cat_fn(struct aws_allocator *allocator, void *ctx) {
    ...
    ASSERT_SUCCESS(aws_byte_buf_init(
        allocator, &destination, str1.len + str2.len + str3.len + 10));
    ...
    ASSERT_BIN_ARRAYS_EQUALS(
        expected, strlen(expected), destination.buffer, destination.len);
...
static int test_buffer_cat_dest_too_small_fn(
...
static int test_buffer_cpy_fn(struct aws_allocator *allocator, void *ctx) {
    ...
    ASSERT_BIN_ARRAYS_EQUALS(
        from.ptr, from.len, destination.buffer, destination.len);
...
static int test_buffer_cpy_offsets_fn(
    struct aws_allocator *allocator,
    void *ctx) {
    (void)ctx;
    ...
    ASSERT_SUCCESS(
        aws_byte_buf_init(allocator, &destination, from_buf.len + 10));
    ASSERT_SUCCESS(aws_byte_buf_append(&destination, &from));
...
static int test_buffer_cpy_dest_too_small_fn(
    struct aws_allocator *allocator,
    void *ctx) {
    (void)ctx;
    ...
    ASSERT_ERROR(
        AWS_ERROR_DEST_COPY_TOO_SMALL,
        aws_byte_buf_append(&destination, &from));
    ASSERT_INT_EQUALS(0, destination.len);
...
AWS_TEST_CASE(
    ...
    test_buffer_cpy_offsets_dest_too_small_fn)
static int test_buffer_cpy_offsets_dest_too_small_fn(
    struct aws_allocator *allocator,
    void *ctx) {
    (void)ctx;
