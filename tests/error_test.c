...
static int s_raise_errors_test_fn(struct aws_allocator *allocator, void *ctx) {
    ...
    ASSERTF_NULL(error, "error should be initialized to NULL");
    ASSERTF_INT_EQUALS(0, aws_last_error(), "error code should be initialized to 0");

    struct aws_error_info test_error_1 = s_errors[0];
    struct aws_error_info test_error_2 = s_errors[1];

    ASSERTF_INT_EQUALS(-1, aws_raise_error(test_error_1.error_code), "Raise error should return failure code.");
    ...
    ASSERTF_INT_EQUALS(-1, aws_raise_error(test_error_2.error_code), "Raise error should return failure code.");
    error = aws_last_error();
    ...
    error = aws_last_error();
    ASSERTF_NOT_NULL(error, "last error should not have been null");
    ASSERTF_STR_EQUALS(
    ...
    aws_reset_error();
    error = aws_last_error();
    ASSERTF_NULL(error, "error should be reset to NULL");
    ASSERTF_INT_EQUALS(0, aws_last_error(), "error code should be reset to 0");
    return 0;
}
...
static int s_error_callback_test_fn(struct aws_allocator *allocator, void *ctx) {
    ...
    ASSERTF_NULL(old_fn, "setting the global error callback the first time should return null");
    aws_raise_error(test_error_1.error_code);

    ASSERTF_NOT_NULL(cb_data.last_seen, "last error should not have been null");
    ASSERTF_TRUE(cb_data.global_cb_called, "Global Callback should have been invoked");
    ASSERTF_FALSE(cb_data.tl_cb_called, "Thread Local Callback should not have been invoked");
    ...
    ASSERTF_NOT_NULL(cb_data.last_seen, "last error should not have been null");
    ASSERTF_FALSE(cb_data.global_cb_called, "Global Callback should not have been invoked");
    ASSERTF_TRUE(cb_data.tl_cb_called, "Thread local Callback should have been invoked");
...
static int s_unknown_error_code_range_too_large_test_fn(struct aws_allocator *allocator, void *ctx) {
    ...
    error = aws_last_error();
    /* error code should still propogate */
    ASSERTF_INT_EQUALS(oor_error_code, error, "Expected error code %d, but was %d", oor_error_code, error);
...
static int s_error_code_cross_thread_test_fn(struct aws_allocator *allocator, void *ctx) {
    ...
    ASSERTF_SUCCESS(
        ...
        aws_last_error());
    ASSERTF_SUCCESS(aws_thread_join(&thread), "Thread join failed with error %d", aws_last_error());
    ...
    ASSERTF_FALSE(test_data.thread_2_id == 0, "thread 2 id should have been set to something other than 0");
    ASSERTF_FALSE(test_data.thread_2_id == test_data.thread_1_id, "threads 1 and 2 should be different ids");
    ASSERTF_INT_EQUALS(
    ...
    ASSERTF_INT_EQUALS(15, test_data.thread_2_code, "Thread 2 code should have matched the thread 2 error.");
    ASSERTF_INT_EQUALS(
