...
static int s_device_rand_buffer_gigantic(struct aws_allocator *allocator, void *ctx) {
    (void)ctx;
    ...
    const size_t bytes_to_check = 50;

    /* Fill dig chunk of zeroed-out memory with randomness */
    uint8_t *memory = aws_mem_calloc(allocator, 1, capacity);
    struct aws_byte_buf buf = aws_byte_buf_from_empty_array(memory, capacity);
    ASSERT_SUCCESS(aws_device_random_buffer(&buf));

    /* Check that start of buffer was filled */
    bool all_zero = true;
    ...
    ASSERT_FALSE(all_zero, "No randomness at start of giant buffer");

    /* Check that end of buffer was filled */
    all_zero = true;
    for (size_t i = capacity - bytes_to_check; i < capacity; ++i) {
        if (memory[i] != 0) {
            all_zero = false;
            break;
        }
    }
    ASSERT_FALSE(all_zero, "No randomness at end of giant buffer");

    aws_mem_release(allocator, memory);
    return AWS_OP_SUCCESS;
}

AWS_TEST_CASE(device_rand_buffer_gigantic, s_device_rand_buffer_gigantic);
