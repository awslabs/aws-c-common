...
static void log_report_fn(const char *tag, const char *log_message) {
    (void)tag;
...
static int test_log_init_clean_up_fn(struct aws_allocator *allocator, void *ctx) {
    ...
    ASSERT_SUCCESS(aws_log(AWS_LOG_LEVEL_TRACE, "log_test", "log_test", "Oh, hello there #1."));
    ASSERT_SUCCESS(aws_log_flush());
    ASSERT_SUCCESS(aws_log(AWS_LOG_LEVEL_TRACE, "log_test", "Oh, hello there #2."));
    ASSERT_SUCCESS(aws_log_flush());
    ASSERT_SUCCESS(aws_log(AWS_LOG_LEVEL_TRACE, "log_test", "Oh, hello there #3."));
    ASSERT_SUCCESS(aws_log(AWS_LOG_LEVEL_TRACE, "log_test", "Oh, hello there #4."));
    ASSERT_SUCCESS(aws_log(AWS_LOG_LEVEL_TRACE, "log_test", "Oh, hello there #5."));
    ASSERT_SUCCESS(aws_log_flush());
...
static int test_log_overflow_message_fn(struct aws_allocator *allocator, void *ctx) {
    ...
    ASSERT_SUCCESS(aws_log(AWS_LOG_LEVEL_TRACE, "log_test", "This message should definitely overflow and get truncated because it is just simply way too long."));
    ASSERT_SUCCESS(aws_log(AWS_LOG_LEVEL_TRACE, "log_test", "Overflow the memory pool, but not the message."));
    ASSERT_SUCCESS(aws_log_flush());
...
void test_log_thread_fn(void *param) {
    ...
    while (*running) {
        if (count < 10) {
            aws_log(AWS_LOG_LEVEL_TRACE, "log_test", "Hello from thread %d!", (unsigned)id);
            ++count;
        }
...
static void log_report_test_order_fn(const char *tag, const char *log_message) {
    (void)tag;
...
void test_log_thread_order_fn(void *param) {
    ...
    while (*running) {
        if (count < 1000) {
            aws_log(AWS_LOG_LEVEL_TRACE, "log_test", "%d %d", index, count);
            ++count;
        }
        aws_thread_current_sleep(1);
    }
...
static int test_log_no_flush_for_no_leaks_fn(struct aws_allocator *allocator, void *ctx) {
    ...
    for (int i = 0; i < 10; ++i)
        AWS_LOG(AWS_LOG_LEVEL_DEBUG, "log_test", "This is a test log.");
...
AWS_TEST_CASE(test_log_thread_basic, test_log_thread_basic_fn)
static int test_log_thread_basic_fn(struct aws_allocator *allocator, void *ctx) {
    (void)ctx;
    ASSERT_SUCCESS(aws_log_system_init(allocator, 128, 1, 1));
    aws_log_set_reporting_callback(NULL);
    aws_log_spawn_log_thread(allocator);
