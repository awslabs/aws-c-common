...
static int cstring_secure_string_length_test_fn(struct aws_allocator *allocator, void *ctx) {
    (void)allocator;
    (void)ctx;

    size_t str_len = 0;
    const char test_string[] = "HelloWorld!";
    ASSERT_SUCCESS(aws_c_string_secure_string_length(test_string, sizeof(test_string), &str_len));
