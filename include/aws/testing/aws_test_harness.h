...
static void *mem_acquire_malloc(struct aws_allocator *allocator, size_t size) {
    struct memory_test_allocator *test_allocator =
        (struct memory_test_allocator *)allocator->impl;

    aws_mutex_lock(&test_allocator->mutex);
    test_allocator->allocated += size;
    struct memory_test_tracker *memory = (struct memory_test_tracker *)malloc(
        size + sizeof(struct memory_test_tracker));
    memory->size = size;
...
#define PRINT_FAIL_INTERNAL(...)                                               \
    cunit_failure_message(                                                     \
        __FUNCTION__, __FILE__, __LINE__, ##__VA_ARGS__, (const char *)NULL)

#define PRINT_FAIL_INTERNAL0(...)                                              \
    cunit_failure_message0(                                                    \
        FAIL_PREFIX,                                                           \
        __FUNCTION__,                                                          \
        __FILE__,                                                              \
        __LINE__,                                                              \
        ...
        if (!(condition)) {                                                    \
            if (!PRINT_FAIL_INTERNAL0(__VA_ARGS__)) {                          \
                PRINT_FAIL_INTERNAL0(                                          \
                    "Expected condition to be true: " #condition);             \
            }                                                                  \
            POSTFAIL_INTERNAL();                                               \
        }                                                                      \
    } while (0)
...
#define ASSERT_FALSE(condition, ...)                                           \
    do {                                                                       \
        if ((condition)) {                                                     \
            if (!PRINT_FAIL_INTERNAL0(__VA_ARGS__)) {                          \
                PRINT_FAIL_INTERNAL0(                                          \
                    "Expected condition to be false: " #condition);            \
            }                                                                  \
            POSTFAIL_INTERNAL();                                               \
        }                                                                      \
    } while (0)
...
#define ASSERT_NOT_NULL(ptr, ...)                                              \
    do {                                                                       \
        ...
        if (!assert_p) {                                                       \
            fprintf(                                                           \
                AWS_TESTING_REPORT_FD,                                         \
                "%sExpected non-null but got null: ",                          \
                FAIL_PREFIX);                                                  \
            if (!PRINT_FAIL_INTERNAL0(__VA_ARGS__)) {                          \
                PRINT_FAIL_INTERNAL0("%s", #ptr);                              \
            }                                                                  \
...
#ifdef _MSC_VER
#    define ASSERT_INT_EQUALS(expected, got, ...)                              \
        ASSERT_TYP_EQUALS(intmax_t, "%lld", expected, got, __VA_ARGS__)
#    define ASSERT_UINT_EQUALS(expected, got, ...)                             \
        ASSERT_TYP_EQUALS(uintmax_t, "%llu", expected, got, __VA_ARGS__)
#else
/* For comparing any signed integer types */
#    define ASSERT_INT_EQUALS(expected, got, ...)                              \
        ASSERT_TYP_EQUALS(intmax_t, "%jd", expected, got, __VA_ARGS__)
/* For comparing any unsigned integer types */
#    define ASSERT_UINT_EQUALS(expected, got, ...)                             \
        ASSERT_TYP_EQUALS(uintmax_t, "%ju", expected, got, __VA_ARGS__)
#endif
...
#define ASSERT_BYTE_HEX_EQUALS(expected, got, ...)                             \
    ASSERT_TYP_EQUALS(uint8_t, "%02X", expected, got, __VA_ARGS__)
#define ASSERT_HEX_EQUALS(expected, got, ...)                                  \
    ASSERT_TYP_EQUALS(unsigned long long, "%llX", expected, got, __VA_ARGS__)
...
#define ASSERT_STR_EQUALS(expected, got, ...)                                  \
    do {                                                                       \
        ...
        if (strcmp(assert_expected, assert_got) != 0) {                        \
            ...
            if (!PRINT_FAIL_INTERNAL0(__VA_ARGS__)) {                          \
                PRINT_FAIL_INTERNAL0(                                          \
                    "ASSERT_STR_EQUALS(%s, %s)", #expected, #got);             \
            }                                                                  \
            POSTFAIL_INTERNAL();                                               \
        }                                                                      \
    } while (0)
...
#define ASSERT_BIN_ARRAYS_EQUALS(expected, expected_size, got, got_size, ...)  \
    do {                                                                       \
        ...
        if (assert_ex_s != assert_got_s) {                                     \
            ...
            if (!PRINT_FAIL_INTERNAL0(__VA_ARGS__)) {                          \
                PRINT_FAIL_INTERNAL0(                                          \
                    "ASSERT_BIN_ARRAYS_EQUALS(%s, %s, %s, %s)",                \
                    #expected,                                                 \
                    #expected_size,                                            \
                    #got,                                                      \
                    #got_size);                                                \
...
#define AWS_TEST_CASE_FIXTURE(name, b, fn, af, c)                              \
    AWS_TEST_CASE_FIXTURE_SUPPRESSION(name, b, fn, af, c, 0)
...
static int aws_run_test_case(struct aws_test_harness *harness) {
    ...
    if (!ret_val) {
        if (!harness->suppress_memcheck) {
            struct memory_test_allocator *alloc_impl =
                (struct memory_test_allocator *)harness->allocator->impl;
            ASSERT_UINT_EQUALS(
